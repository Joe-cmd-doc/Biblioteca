<x-app-layout>
    <div class="container mx-auto py-12 px-6">
        <h1 class="text-3xl font-bold mb-6 text-black">Edit Review</h1>

        <form action="{{ route('reviews.update', $review->id) }}" method="POST" class="space-y-6 bg-white p-6 rounded-lg shadow-md">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="4" required
                          class="mt-1 block w-full rounded-md shadow-sm border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 bg-white text-black border-gray-300">{{ old('content', $review->content) }}</textarea>
            </div>
